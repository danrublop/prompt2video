...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { prompt, aspectRatio = '16:9', duration = 150, language = 'English', voice = 'alloy' } = body
    ...
    console.log(`Starting whiteboard video generation for: "${prompt}"`)

    // Step 1: Generate Script with GPT
    console.log('Step 1: Generating script with GPT...')
    ...
    console.log('Script scenes:', script.scenes.map(s => ({ id: s.sceneId, narration: s.narration.substring(0, 50) + '...' })))

    // Step 2: Generate Whiteboard Plan and Assets for each scene
    console.log('Step 2: Generating whiteboard assets...')
    const whiteboardScenes = []
    
    for (let i = 0; i < script.scenes.length; i++) {
      const scene = script.scenes[i]
      console.log(`Processing scene ${i + 1}/${script.scenes.length}: ${scene.sceneId}`)
    ...
    console.log('Whiteboard video generation completed successfully!')
    ...
    return NextResponse.json({
      ...
      script: script,
      totalCost: calculateTokenCost(1000, 500) + (script.scenes.length * 0.08), // Approximate cost
      duration: script.totalDuration
    })

  } catch (error) {
    clearTimeout(timeout)
    console.error('Whiteboard video generation failed:', error)
    ...
    return NextResponse.json(
      { 
        error: 'Whiteboard video generation failed', 
